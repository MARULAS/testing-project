...
public class ShoppingCart {
    ...
    public void addItem(Product product, int quantity) {
        ...
        for (CartItem item : items) {
            if (item.getProduct().getId().equals(product.getId())) {
                ...
                // Post-condition: cart still has the product, count didn't decrease
                assert hasProduct(product.getId()) : "cart must contain the product after addItem";
                assert items.size() >= previousCount : "cart size must not decrease";
                assert total() >= 0 : "INVARIANT: total must be >= 0";
                return;
            }
        }
        items.add(new CartItem(product, quantity));
        // Post-condition: cart has the product, count increased or stayed same
        assert hasProduct(product.getId()) : "cart must contain the product after addItem";
        assert items.size() >= previousCount : "cart size must not decrease";
        assert total() >= 0 : "INVARIANT: total must be >= 0";
    }
    ...
    public void removeItem(String productId) {
        ...
        assert total() >= 0 : "INVARIANT: total must be >= 0 after removeItem";
    }
    ...
    public void updateQuantity(String productId, int quantity) {
        ...
        for (CartItem item : items) {
            if (item.getProduct().getId().equals(productId)) {
                ...
                assert total() >= 0 : "INVARIANT: total must be >= 0 after updateQuantity";
                return;
            }
        }
    ...
    public double applyDiscount(double discountRate) {
        ...
        // Post-condition: if discount > 0, result should be strictly less than raw total
        // (unless rawTotal is 0, then they are equal)
        assert discountRate == 0 || rawTotal == 0 || discounted < rawTotal
                : "post-condition: discounted total must be less than raw total when discount > 0 and total > 0";
        assert discounted >= 0 : "post-condition: result must be >= 0";
        assert total() >= 0 : "INVARIANT: total must be >= 0 after applyDiscount";
        return discounted;
    }
    ...
    public void clear() {
        items.clear();
        assert total() >= 0 : "INVARIANT: total must be >= 0 after clear";
    }

    /** Helper to check if a product is in the cart (used for post-conditions) */
